	...
	try {
		const offers = await Offer.find()
			...
			.populate({
				...
				model: "Product",
				select:
					"name imageUrl category stock packSize quantity retailerPrice marketPrice dealerPrice",
			})
...
export const getWinningOffersForSeller = async (req, res) => {
	try {
		...
		const winningOffers = await Offer.find({ winner: sellerId })
			.select("title description startDate endDate status  products createdAt ")
			.populate({
				path: "winner",
				select: "name email phone companyName",
			})
	...
	try {
		...
		const offersWithWinners = await Offer.find({
			...
			.populate({
				path: "products.productId",
				select:
					"name imageUrl marketPrice dealerPrice retailerPrice packSize quantity",
			})
			.sort({ createdAt: -1 })
...
export const getOfferDashboardData = async (req, res) => {
	try {
		const { role } = req.body; // dealer / retailer
		const now = new Date();

		const [currentOffers, previousWinners] = await Promise.all([
			...
			// 🏆 PREVIOUS OFFER WINNERS
			Offer.find({
				offerFor: role,
				status: "closed",
				winner: { $ne: null },
			})
				.populate("winner", "name companyName")
				.select("title endDate winner")
				.sort({ endDate: -1 })
				.limit(5),
		]);
